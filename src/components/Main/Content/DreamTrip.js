...
const DreamTrip = ({dreamTitle, dreamContent}) => {
...
return (
    ...
    {
          dreamTitle.map((item) => (
                <div class="flex flex-col items-start gap-2 pl-6 lg:pl-0 h-40 justify-end">
        ...
        <div ref={scrollRef} onScroll={() => {checkScroll(); handleRightScrollBool(); handleLeftScrollBool()}} class="flex flex-row flex-nowrap overflow-x-scroll scrollbar-hide gap-4 w-full pl-6 lg:pl-0 pr-6">
    {
        dreamContent.map((item) => (
            <div class="min-w-[calc(100%/2)] md:min-w-[calc(100%/3)] lg:min-w-[calc(25%)] xl:min-w-[calc(25%-6px)] relative group hover:cursor-pointer">
                <div>
                    <img class="w-full h-full group-hover:opacity-80 transition duration-200 ease-in-out" src={item.image} alt={item.location}/>
                </div>
